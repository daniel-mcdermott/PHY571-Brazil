...
#Control Panel
N = 50           # Number of Disks, set here
Rad = 0.04       # Radius of Most Disks
Mass = 1         # Radius of Most Disks
Odd_Rad = 0.08   # Brazil Nut Radius
Odd_Mass = 2     # Brazil Nut Mass
g = 30.81        # Set to 0 to turn gravity off, set to any positive value to turn gravity on at various strengths
F = 20
A=0.9

msgreen = 22     # For every 0.02 in radius us ms 11
msblue = 44
...
class Disks:
    ...
    def step(self, time_step, frequency, amplitude, g):
        """Make a time step with gravity and shaking"""
        i = 0
        # Calculate the current shaking force
        if i >= 8:
            shaking_force = np.sin(frequency * time_step) * amplitude
        else:
            shaking_force = 0
        # Apply gravity and shaking force
        self.velocities = self.velocities + np.array([0, -(g + shaking_force)]) * time_step
        for i in range(self.num_disks):
            self.update_collision_events(i)
    ...
    def handle_collision(self, indices):
        ...
        if indices[1] < 4:
            ...
            restitution_coefficient = 0.65  # Adjust the coefficient of restitution as needed
